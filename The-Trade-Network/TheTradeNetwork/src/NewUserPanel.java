...
public class NewUserPanel extends JPanel implements StringConstants {
	...
	public boolean checkValues(String[] info){
		boolean correct = true;
		//username wrong
		if(info[0].length() < 5){
			correct = false;
			JOptionPane.showMessageDialog(mainpanel, "username too short");
		}
		else if(!info[1].equals(info[7])){
			correct = false;
			JOptionPane.showMessageDialog(mainpanel, "passwords do not match up");
		}
		else if(info[4].length() != 10){
			correct = false;
			JOptionPane.showMessageDialog(mainpanel, "error at phonenumber field\n please look over - parenthesis not allowed");
		}
		else if(info[2].length() != 5){
			correct = false;
			JOptionPane.showMessageDialog(mainpanel, "error at zipcode");
		}
		return correct;
	}
	
	public String trimPhone(String pn){
		String phone = pn;
	...
	class NewUserPanelListener implements ActionListener {
		...
		public void createErrorMessage(int errorcode) {
			...
			if (errorcode == 547) {
				JOptionPane
						.showMessageDialog(mainpanel,
								"Error occured, field incorrect, please look over information");
			}
