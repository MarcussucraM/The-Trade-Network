...
public class SearchPanel extends JPanel implements StringConstants {
	...
	public SearchPanel(TheMainPanel mainpanel, DBConnection db) {
		...
		btnGoBack.setBounds(616, 481, 148, 59);
		add(btnGoBack);
		...
		btnOfferItem.setBounds(318, 481, 148, 59);
		add(btnOfferItem);
		btnOfferItem.addActionListener(new ButtonListener());

		JTabbedPane tabbedPane = new JTabbedPane(JTabbedPane.TOP);
		tabbedPane.setBackground(Color.WHITE);
		tabbedPane.setBounds(23, 105, 741, 365);
		add(tabbedPane);

		try {
			model = new ProjectTableModel(searchtableColumns, db.fillSearchTable());
		...
		btnSearch.setBounds(23, 481, 148, 59);
		add(btnSearch);
					...
					if (items.length != 0) {
						...
						if (JOptionPane.showConfirmDialog(
							...
							try {
								...
								JOptionPane.showMessageDialog(mainpanel, "You're already trading that item for something else!");
								e1.printStackTrace();
							}
						}
					}
