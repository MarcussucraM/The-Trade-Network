...
public class MyTrades extends JPanel implements StringConstants {
	...
	public MyTrades(TheMainPanel mainpanel, DBConnection db) {
		...
		tabbedPane.setBounds(197, 80, 545, 468);
		add(tabbedPane);
		...
		JLabel lblMyTrades = new JLabel(mainpanel.getUserName() + "'s" + " Trades");
		lblMyTrades.setFont(new Font("Tahoma", Font.PLAIN, 27));
		lblMyTrades.setBounds(297, 36, 300, 33);
		add(lblMyTrades);
		...
		public void actionPerformed(ActionEvent e) {
			...
			// if user presses removeitem button
				...
				if (tabbedPane.getSelectedIndex() == 0) {
					...
					int row = trades_table.getSelectedRow();
					...
					}else{ JOptionPane.showMessageDialog(mainpanel, "You need to select a row first!"); }
			...
			// if user presses myoffers button
				...
				mainpanel.getAcceptedOffersPanel().refreshPage();
				CardLayout cl = (CardLayout) (mainpanel.getLayout());
			...
			//if user presses accept offer button
				...
				if (tabbedPane.getSelectedIndex() == 2) {
					...
					if(row != -1){
						...
						String[] rowData = model3.getValuesAt(row);
						
						if(JOptionPane.showConfirmDialog(mainpanel, "Are you sure you want to\nAccept this Offer?: " + rowData[2] + " for " + rowData[0]) == JOptionPane.YES_OPTION){
							try {
								//accept and refresh page
								db.acceptItem(rowData);
								refresh_page();
								
							} catch (SQLException e1) {
								// TODO Auto-generated catch block
								JOptionPane.showMessageDialog(mainpanel, "You need to select a row first!");
								e1.printStackTrace();
							}
						}
					} else { JOptionPane.showMessageDialog(mainpanel, "You have to select a row first!"); }
